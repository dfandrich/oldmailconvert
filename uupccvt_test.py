...
class TestUupcCvt(unittest.TestCase):

    def run_inout_test(self, infn, outfn):
        outputfile = io.TextIOWrapper(io.BytesIO())
        with open(infn, 'r') as inputfile, \
            mock.patch('uupccvt.sys.stdin', new=inputfile), \
                mock.patch('uupccvt.sys.stdout', new=outputfile):
            uupccvt.main()
    ...
    def test_message_2(self):
        self.run_inout_test('testdata/uupc.input.2', 'testdata/uupc.expected.2')
